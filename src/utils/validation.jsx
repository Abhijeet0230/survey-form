const validate = (values) => {
  ...
  if (values.surveyTopic === "Technology") {
    ...
    if (!values.yearsOfExperience) {
      ...
      isNaN(values.yearsOfExperience) ||
  ...
  values.additionalQuestions.forEach((question, index) => {
    if (!values[`additionalQuestion${index}`]) {
      errors[`additionalQuestion${index}`] = `${question.label} is required`;
    }
  });
